...
class App extends Component {
  state = {
    score: 0,
    timer: 60,
    randomImgIndex: Math.ceil(Math.random() * imagesList.length),
  ...
  getActiveTabThumbnail = () => {
    const {activeTabId} = this.state
    console.log(activeTabId)
    const filterActiveImgs = imagesList.filter(
      eachImg => eachImg.category === activeTabId,
    )
    return filterActiveImgs
  }

  playing = currentId => {
    const {randomImgIndex} = this.state
    const {id} = imagesList[randomImgIndex]
    if (currentId === id) {
  ...
  render() {
    const {score, timer, randomImgIndex, activeTabId, isPlay} = this.state
    const {imageUrl, category} = imagesList[randomImgIndex]
    const filterActiveImgs = this.getActiveTabThumbnail()
    console.log(filterActiveImgs)
    return (
      <div>
        ...
        <div className="container">
          {isPlay ? (
            <>
              <img src={imageUrl} alt={category} className="match" />
              <ul className="list-container-buttons">
              ...
              <ul className="list-container-thumbnails">
                {filterActiveImgs.map(eachThumbnail => (
                  <Thumbnail
                    activeThumbnails={eachThumbnail}
